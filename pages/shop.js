    ...
    return (
        <ContainerBlock title="Shop">
            ...
            <div className='my-10 p-10 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-5'>
                {firedata.map((data) => {
                    return(
                        <a href={`/product/${data.id}`} key={data.id} className="glassmorph md:mx-16 mx-6 w-[18rem] lg:w-[26rem] xl:w-[35rem] rounded-lg hover:scale-105 transition-scale duration-300 my-6">
                            ...
                            <Link href={`/checkout/shop`}><button className='my-4 button px-4 py-3 rounded-md block mx-auto'>Buy Now</button></Link>
                        </a>
                    )
                })}
            </div>
